...
function ShopOwnerDetailView() {
  ...
  const shops = useMemo(() => {
    ...
    return allShops
      .filter((s) => String(s.shopOwnerId) === String(ownerId))
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Button asChild>
            <Link href={`/master/shop-owners/edit/?id=${ownerId}`}>
        ...
        <CardContent className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-primary text-primary-foreground text-lg font-semibold">
            {owner.profileImageUrl ? (
              <Avatar src={owner.profileImageUrl} alt={owner.name} />
            ) : (
              initials
            )}
          </div>
...
function ShopRow({
  ...
  const billingType = s.settings?.billingType ?? shop.billingType;
  const gstRequired = billingType === "GST" || billingType === "BOTH";
  // Front image may live on the dedicated column or the legacy settings.documents.
  const frontImageUrl = shop.frontImageUrl ?? docs.frontImageUrl;
  // Only flag documents that are actually required and absent: GST certificate
  // is required for GST / Both billing only; Udyam is optional so it is never
  // flagged as "missing".
  const missingDocs: string[] = [];
  if (!frontImageUrl) missingDocs.push("Front Image");
  if (gstRequired && !docs.gstCertificateUrl) missingDocs.push("GST Cert");

  const displayName = s.shopName ?? s.name ?? "Unnamed";
  const displayMobile = s.contactMobile ?? s.mobile ?? "—";
  const addressParts = [
    shop.address?.addressLine,
    shop.address?.street ?? shop.settings?.street,
    shop.address?.area ?? shop.area,
    shop.address?.taluk ?? shop.taluk,
    shop.address?.district ?? shop.district,
    shop.address?.state ?? shop.state,
    shop.address?.pincode ?? shop.pincode,
  ]
    .filter(Boolean)
    .join(", ");
